  ...
  methods: {
    dropAction(arr, dragResult, column) {
      ...
      if (addedIndex !== null) {
        ...
        if (payload.status.length > 1) { this.$emit('item-dropped', { item: payload, column: status }); } else if (payload.status !== status) {
...
<template>
  <div class="kanban">
    <template v-if="!loading">
      ...
      >
        <div class="kanban__header">
          ...
          <p
            v-if="!innerColCount"
            class="kanban__quantity"
          >
            {{ column.columnItems.length }} {{ countText }}
          </p>
        </div>
        <div
          class="kanban__content"
          :style="contentStyle"
        >
          <p
            v-if="innerColCount"
    ...
    <div
      v-else
      class="kanban__skeleton-container"
    >
      <slot name="skeleton" />
      <div
        v-if="!$slots.skeleton"
        class="kanban__skeleton"
      >
        <div
